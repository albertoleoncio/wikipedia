...
<?php
//Códigos
$abre_link = "<li class=\"w3-padding-small w3-left-align\" style=\"cursor:pointer;\" onclick=\"window.open('https://pt.wikipedia.org/w/index.php?";
...
if ($_GET["conta"]) {
	...
	*/
	...
	if ($result['-1']) {
		//Criar página raiz
		echo $abre_link."action=edit&preload=Template%3ADB1%2FPreload&title=Wikipedia%3APedidos+a+administradores%2FDiscuss%C3%A3o+de+bloqueio%2F".urlencode($conta)."&preloadparams%5B%5D=".urlencode($evidence)."&preloadparams%5B%5D=".urlencode("{{subst:#if:".$defesa."|{{citação2|1=".$defesa."|2={{subst:#if:".$diff."|{{dif|".$diff."}}}}}}}}")."', '_blank')\">Criar DB</li>\n";
		echo $abre_link."title=Wikip%C3%A9dia:Pedidos_a_administradores/Discuss%C3%A3o_de_bloqueio/Lista_de_pedidos&action=edit&section=new&nosummary=1&preload=Wikip%C3%A9dia:Pedidos_a_administradores/Discuss%C3%A3o_de_bloqueio/Lista_de_pedidos/Preload&preloadparams%5B%5D=".urlencode($conta)."&preloadparams%5B%5D=".urlencode($conta)."', '_blank')\">Publicar DB na lista de pedidos</li>\n";
		...
		echo $abre_link."title=Predefini%C3%A7%C3%A3o:MRConduta&action=edit', '_blank')\">Colar (substituir) novo código no painel</li>";
		if ($sysop) {
			echo $abre_link."title=Especial:Mensagens_em_massa&spamlist=Wikip%C3%A9dia%3APedidos%2FDiscuss%C3%A3o+de+bloqueio%2FMassmessage&subject=Discuss%C3%A3o+de+bloqueio+de+".urlencode($conta)."&message=%7B%7Bsubst%3AUsu%C3%A1rio%3ATeles%2FMassMessage%2FDesbloqueio%7C".urlencode($conta)."%7C%7C~~~~~%7C%7D%7D', '_blank')\">Enviar mensagens em massa</li>";
		} else {
			echo $abre_link."title=Wikip%C3%A9dia:Pedidos/Outros&action=edit&section=new&preloadtitle=Mensagens+em+massa+para+discussão+de+bloqueio+do+usuário+".urlencode($conta)."&preload=Wikip%C3%A9dia:Pedidos/Outros/PreloadMassMessageDB&preloadparams%5B%5D=".urlencode($conta)."&preloadparams%5B%5D=".urlencode("https://pt.wikipedia.org/w/index.php?title=Especial:Mensagens_em_massa&spamlist=Wikip%C3%A9dia%3APedidos%2FDiscuss%C3%A3o+de+bloqueio%2FMassmessage&subject=Discuss%C3%A3o+de+bloqueio+de+".urlencode($conta)."&message=%7B%7Bsubst%3AUsu%C3%A1rio%3ATeles%2FMassMessage%2FDesbloqueio%7C".urlencode($conta)."%7C%7C%7E%7E%7E%7E%7E%7C%7D%7D")."', '_blank')\">Solicitar mensagens em massa</li>\n";
		}
		echo $abre_link."title=User_talk:".urlencode($conta)."&action=edit&section=new&nosummary=1&preload=Predefini%C3%A7%C3%A3o:Notifica%C3%A7%C3%A3o_de_discuss%C3%A3o_de_bloqueio/Preload', '_blank')\">Enviar notificação ao usuário</li>\n";
	} else {
		$desambig = FALSE;
		...
		if (!$desambig) {
			...
			echo $abre_link."action=edit&preload=Template%3ADB1%2FPreload&title=Wikipedia%3APedidos+a+administradores%2FDiscuss%C3%A3o+de+bloqueio%2F".urlencode($conta)."%2F2&preloadparams%5B%5D=".urlencode($evidence)."&preloadparams%5B%5D=".urlencode("{{subst:#if:".$defesa."|{{citação2|1=".$defesa."|2={{subst:#if:".$diff."|{{dif|".$diff."}}}}}}}}")."', '_blank')\">Criar DB/2</li>\n";
			...
			echo $abre_link."title=Wikip%C3%A9dia:Pedidos_a_administradores/Discuss%C3%A3o_de_bloqueio/".urlencode($conta)."&action=edit', '_blank')\">Criar desambiguação</li>";
			if ($sysop) {
			...
			//Criar página /$count
			echo $abre_link."action=edit&preload=Template%3ADB1%2FPreload&title=Wikipedia%3APedidos+a+administradores%2FDiscuss%C3%A3o+de+bloqueio%2F".urlencode($conta)."%2F".$count."&preloadparams%5B%5D=".urlencode($evidence)."&preloadparams%5B%5D=".urlencode("{{subst:#if:".$defesa."|{{citação2|1=".$defesa."|2={{subst:#if:".$diff."|{{dif|".$diff."}}}}}}}}")."', '_blank')\">Criar DB</li>\n";
