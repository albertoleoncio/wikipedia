...
if ($_GET["artigo_titulo"]) {
	...
	foreach ($editores_artigo as $list) {
		...
		echo "<a target='_blank' href='https://pt.wikipedia.org/w/index.php?title=User_talk:".urlencode($list['name'])."&action=edit&section=new&preloadtitle=".urlencode("Aviso sobre a eliminação da página [[".$list['name']."]] ([https://alberobot.toolforge.org/siw.php ESR-SIW])")."&preload=Predefini%C3%A7%C3%A3o:Aviso-ESR-SIW/Preload&preloadparams%5b%5d=".urlencode(trim($_GET["artigo_titulo"]))."&preloadparams%5b%5d='>".$list['name']."</a>";
